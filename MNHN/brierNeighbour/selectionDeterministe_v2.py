        ...
        if accession_num in dico_exemple: 
            ...
            if nb_ex_test != 0:
                ...
                if valid_interval_ensemble != []: 
                    ...
                    # récupération des paires de séquences valides du seed (hors contextuel)
                    list_pair_seq_name = tuple(dico_seq.keys())
                    # récupération du poid de chacune des paires de séquence
                    ...
                    while example_selected_count < nb_ex_test:
                        list_example, example_selected_count = randomOneSeqExempleSelection(list_example, dico_seq, valid_interval_ensemble, list_pair_seq_name, weights_list, 
...
def randomOneSeqExempleSelection(list_example, dico_seq, valid_interval, list_pair_seq_name, weights_list, 
                                 context_kl, context_kr, context_pl, context_pr, example_selected_count):  
    """
    ...
    """
    # selection aléatoire d'une paire de séquence valide (pondérée par le nombre de couple d'acides aminés valides dans la paire de séquence)
